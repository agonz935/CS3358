...
{
    if(new_capacity < 1)  //protects against a capacity of zero
    ...
    int* new_data = new int[capacity]; //dynamic int array allocated

    for(int i = 0; i < used; ++i)
        new_data[i] = data[i];

    delete [] data; //deallocates memory of previous data array
...
bool IntSet::add(int anInt)
{
   ...
   {
        if(used >= capacity) //resizing array based on user input
            resize(int(capacity * 1.5) + 1);

        data[used] = anInt;  //adds int to end
        ++used;
        return true;
   }
...
bool operator==(const IntSet& is1, const IntSet& is2)
{
   ...
   else if(is1.isEmpty() && is2.isEmpty())
    return true;
